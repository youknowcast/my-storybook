				...
				>
					<div>
						...
						<Button
							...
							size={4}
							label="削除(全部)"
							onClick={() => onClickDeleteSectionAndItems(section)}
						/>
						<Button
							primary={false}
							backgroundColor="gray"
	...
	return (
		<div>
			{items.map((item, index) => {
				const children =
					"items" in item
						? (item as Section).items.map((entry, idx) => (
								// biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
								<div key={idx}>-- {entry.id}</div>
							))
						: undefined;
				return (
					<>
...
const DnDSingleList = (props: DnDSingleListProps) => {
	...
	const itemsToFlatItems = (items: Item[]) => {
		return items.flatMap((entry, index) => {
			if ("items" in entry) {
				...
				const children = section.expanded
					? section.items.map((entry, idx) =>
							withIndex(entry, nextDndIndex(), idx, section.index),
						)
					: [];
	...
	const onDragEnd = (result: DropResult) => {
		...
		const dstItem = flatItems.find(
			(entry) => entry.index === destination.index,
		);
		if (srcItem === undefined || dstItem === undefined)
			throw new Error("dnd src and dst item are required");

		const newItems = Array.from(items);

		const parentSection = (item: Item) => {
			if (!isSection(item) && item.parentIndex !== undefined) {
				const section = flatItems.find(
					(entry) => entry.index === item.parentIndex,
				) as Section;
		...
		if (isSection(srcItem)) {
			...
			const resolveDstSection = (
				srcItem: Item,
				dstItem: Item,
			): [Section | undefined, "toUp" | "toDown"] => {
				...
				// 下部から上部への移動
				const dstParent = parentSection(dstItem);
				const sibling = flatItems.find(
					(entry) => entry.index === dstItem.index! - 1,
				);
			...
			const dstSectionItemsLocalIndex = (
				dstItem: Item,
				newItems: Item[],
			...
			if (srcParent !== undefined) {
				if (dstParent !== undefined && srcParent.index === dstParent.index) {
					...
					newDstItems.splice(
						dstSectionItemsLocalIndex(dstItem, newDstItems, direction),
						0,
						removed,
					);
				...
				newSectionItems.splice(
					dstSectionItemsLocalIndex(dstItem, newSectionItems, direction),
					0,
					removed,
				);
	...
	const onClickDeleteSectionAndItems = (target: Section) => {
		...
		setItems((items) => {
			return items.filter(
				(item) => !(isSection(item) && item.id === target.id),
			);
		});
	};
		...
		<DragDropContext onDragEnd={onDragEnd}>
						...
						{flatItems.map((item) => {
							if ("items" in item) {
								return (
									<SectionComponent
										...
										onClickDeleteSectionAndItems={onClickDeleteSectionAndItems}
							...
							if (field.parentIndex !== undefined) {
								const parent = flatItems.find(
									(entry) => entry.index === field.parentIndex,
								);
								if (parent === undefined)
									throw new Error("parent section is not found");
								return (parent as Section).expanded ? (
									<FieldComponent
										key={field.id}
										field={field}
										parentSectionIndex={field.parentIndex}
									/>
								) : undefined;
							}
							return (
			...
			<Button
				...
				onClick={() =>
					setItems((items) => [
						...items,
						{ id: `dummy-${Math.floor(Math.random() * 1000)}`, content: "aa" },
					])
				}
			/>
			<Button
				...
				onClick={() =>
					setItems((items) => [
						...items,
						{
							id: `dummy-section-${Math.floor(Math.random() * 1000)}`,
							content: "aa",
							items: [],
						},
					])
