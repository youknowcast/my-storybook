...
const initialItems: Item[] = [
  ...
  { id: 'section-1', content: 'Section 1', items: [{ id: 'item-5', content: 'Item 5' }, { id: 'item-6', content: 'Item 6' }] },
  { id: 'section-2', content: 'Section 2', items: [{ id: 'item-8', content: 'Item 8' }, { id: 'item-9', content: 'Item 9' }] },
            ...
            {(provided, snapshot) => (
              ...
              >
                {section.items.map((field, fieldIndex) => (
                  <FieldComponent key={field.id} index={fieldIndex} field={field} />
                ))}
...
const DnD: React.FC = () => {
  ...
  const moveBetweenSections = (result: DropResult, srcSectionIndex: number, dstSectionIndex: number) => {
    const { source, destination, type } = result;
    ...
    newSections[dstSectionIndex] = { ...destinationSection, items: destinationItems };
    setItems(newSections);
  }
  ...
  const moveBaseToSection = (result: DropResult, dstSectionIndex: number) => {
    ...
    newItems[dstSectionIndex] = { ...destinationSection, items: destinationItems };
    setItems(newItems);
  }
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (type === 'base') {
      ...
      const sourceSectionIndex = items.findIndex((item) => 'items' in item && item.id === source.droppableId);
      const destinationSectionIndex = items.findIndex((item) => 'items' in item && item.id === destination.droppableId);
      ...
      if (sourceSectionIndex >= 0 && destinationSectionIndex >= 0) {
        if (sourceSectionIndex === destinationSectionIndex) {
          ...
          moveBetweenSections(result, sourceSectionIndex, destinationSectionIndex);
        }
      ...
      <Droppable droppableId="droppable" type="base">
        {(provided) => (
          <div
            {...provided.droppableProps}
            ref={provided.innerRef}
            style={{ padding: '8px', width: '250px', border: '1px solid lightgrey' }}
          >
